...
public class IntentWrapper {
    ...
    public static List<IntentWrapper> getIntentWrapperList() {
        if (sIntentWrapperList == null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                PowerManager powerManager = (PowerManager) DaemonEnv.sApp.getSystemService(Context.POWER_SERVICE);
                boolean ignoringBatteryOptimizations = powerManager.isIgnoringBatteryOptimizations(DaemonEnv.sApp
                        .getPackageName());
    ...
    public static List<IntentWrapper> whiteListMatters(final Activity activity, String reason) {
        final List<IntentWrapper> showed = new ArrayList<>();
        if (reason == null) {
            reason = "核心服务的持续运行";
        }
        List<IntentWrapper> intentWrapperList = getIntentWrapperList();
        for (final IntentWrapper wrapper : intentWrapperList) {
            if (!wrapper.doesActivityExists()) {
                continue;
            }
                        ...
                        new AlertDialog.Builder(activity)
                                ...
                                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        wrapper.startActivitySafely(activity);
                                    }
                                }).show();
                        showed.add(wrapper);
                    }
